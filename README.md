...
# **Karabo Oliphant**
### Full-Stack & ML Engineer · Johannesburg, South Africa 🇿🇦
...
> **I ship production-grade systems — fintech, ML, and everything in between.** From micro-lending platforms to PPO trading agents running on Kubernetes, I build software that moves money, data, and models — securely, at scale, and fast.
...
|---------|-------|--------------|
| **LendFlow** | `.NET 8` `React` `Tailwind` `SQL Server` `Redis` | Micro-lending platform with real-time credit scoring and loan lifecycle management |
| **PayFlow** | `.NET 8` `React` `SQL Server` `Redis` | Payment processing system with idempotency, retries, and transaction reconciliation |
| **recon-engine** | `.NET 8` `XGBoost` `SQL Server` `Hangfire` | Financial reconciliation with ML-powered matching — Exact → Fuzzy → ML → RuleBased. **0.99 ROC AUC** |
...
| **kyc-service** | `.NET 8` `Azure Blob Storage` | FICA-compliant customer onboarding with document verification |

### 🧠 ML & MLOps

| Project | Stack | What It Does |
|---------|-------|--------------|
| **rl-trading-agent** | `Python` `PyTorch` `SB3` `FastAPI` `K8s` `GHCR` | PPO reinforcement learning trading agent. Deployed on **k3s Kubernetes** with multi-stage Docker slim build (4GB → 521MB), GCR-published, Prometheus/Grafana monitoring, MLflow experiment tracking |
...
| **ml-platform** | `Python` `FastAPI` `PostgreSQL` `Docker` | Self-service MLOps platform — register, version, deploy, and monitor ML models via CLI or API |
| **ml-fraud-detection** | `Python` `Kafka` `MLflow` `MinIO` `DVC` `Prometheus` `Grafana` | Full MLOps pipeline with streaming data, experiment tracking, data versioning, model drift detection, and 72 passing tests |
| **ML Monitoring Stack** | `Prometheus` `Grafana` `Alertmanager` `Docker` | Real-time ML observability — model metrics, drift detection, CPU/memory alerts on deployed agents |

### 🌐 Other

| Project | Stack | What It Does |
|---------|-------|--------------|
| **FD-Marketplace** | `Node.js` `React` `Kafka` `Stripe` `PostgreSQL` | Food delivery marketplace with microservices architecture and payment orchestration |
| **EduTrack** | `Vite` `React` `TypeScript` `Tailwind` | Real-time class attendance and student progress tracking |
...
![Stable-Baselines3](https://img.shields.io/badge/SB3-RL-green?style=for-the-badge&logo=python&logoColor=white)
...
- **Stack Choice is Strategic** — You need distributed training? PyTorch + Ray. You need low-latency inference? ONNX + FastAPI. You need regulatory compliance? .NET + SQL Server. I pick the right tool for the constraints.
